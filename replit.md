...
- Script generator (server-side): `script-generator.ts` in `src/modules/` detects standalone script/CLI requests (Python, Go, Rust, Node.js, TypeScript) via `detectStandaloneScript()` and generates language-appropriate project files. Integrated into `conversation-phase-handler.ts` — runs before `isProjectCreationRequest()` and `handleLocalResponse()` in both `initial` and `complete`/`editing` phases, preventing script requests from falling through to the knowledge base search.
...
- Prompt Generator: `prompt-enhancer.ts` powers `POST /api/prompt/generate` and `POST /api/prompt/upgrade`. Matching priority: (1) `TOPIC_TEMPLATES` key match — 17 named templates for cybersecurity (vapt, pentest, security scanner), DevOps (ci/cd, monitoring, devops), content (cms, knowledge base), finance (expense, erp), support (helpdesk, lms), bug tracking (bug tracker, issue tracker), and original 3; (2) `detectDomainFromText()` from `domain-knowledge.ts`; (3) all-domains keyword scan; (4) `buildPromptFromKeywords()` fallback with 70+ `inferEntitiesFromTopic` mappings + `acronymEntityMap` for common tech acronyms (vapt, pentest, devops, cms, erp, api, iot, ml, etc.). Avoids "X management management platform" duplication bug.
