...
| `prompt-enhancer.ts` | Prompt Generator used by `POST /api/prompt/generate` and `POST /api/prompt/upgrade`. Expands short topics into full structured prompts. Matching priority: (1) `TOPIC_TEMPLATES` exact/substring key match — 17 named templates covering cybersecurity (vapt, pentest, security scanner), DevOps (ci/cd, monitoring, devops), content (cms, knowledge base), finance (expense, erp), support (helpdesk, lms), bug tracking (bug tracker, issue tracker), and the original three (pet adoption, restaurant ordering, fitness tracker); (2) `detectDomainFromText()` from `domain-knowledge.ts`; (3) loop over all `IndustryDomain` keyword sets; (4) `buildPromptFromKeywords()` fallback — uses `ENTITY_KEYWORDS`, `FEATURE_KEYWORDS`, `WORKFLOW_INDICATORS`, and `inferEntitiesFromTopic()` which covers 70+ domain keywords and a secondary `acronymEntityMap` (vapt, pentest, devops, cms, erp, api, iot, ml, etc.) before falling back to word extraction. Avoids "X management management platform" duplication by detecting if topic already contains "management". |
