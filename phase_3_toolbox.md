# Execution Plan - Phase 3: Agent Toolbox (Tool Registry)

Phase 3 establishes a central, strongly-typed tool registry (`toolbox.ts`) that maps tool names and descriptions to safe backend functions (`vfs.ts` and `linter.ts`). It provides permission filtering per agent and formats tool schemas for Ollama API calls.

This updated plan includes robust parameter type validation, safe type coercion (casting), exception safety wrappers, and JSON schema compatibility mapping (specifically mapping integer types for line index parameters).

---

## Prerequisites & Dependencies

- Phase 1 VFS (`vfs.ts`) completed.
- Phase 2 Linter (`linter.ts`) completed.

---

## Detailed Step-by-Step Instructions

### Step 1: Implement Toolbox Module
Create file: [`src/lib/agents/ruflo/toolbox.ts`](file:///c:/Users/Lenovo/Desktop/AutoCoder/src/lib/agents/ruflo/toolbox.ts)
...
1. Call `executeTool('apply_diff', { file_path: 'test.ts', start_line: '2', end_line: '2', new_content: 'a' }, 'convo-id')` with string representation of lines, verifying they are coerced to integer types and run successfully without type errors.
2. Call `executeTool('read_file', { file_path: 'nonexistent.ts' }, 'convo-id')` and assert it returns a cleanly wrapped error object with `{ found: false }` and recovery suggestions.
3. Verify `toolToOllamaFormat(TOOL_REGISTRY[2])` outputs `integer` JSON schema type specifications for both `start_line` and `end_line` keys.
4. Pass a non-boolean invalid argument for a boolean check and verify it triggers a validation error instead of crashing.
